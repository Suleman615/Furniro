...
export default function ProductDetail() {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const productId = await localStorage.getItem('productDetail');
                // Fetch the product details based on the productId
                // Set the product in state
                let detail = products.filter((p) => p.webID === productId)[0];
                setCurrentColor(detail.displayColor[0].replace(' ', '_'))
                setProduct(detail);
                setLoaded(true)
            } catch (error) {
